...
// this canvas size, rather than runtime-sized (std::vector) - mirrors
// pw-video-compositor/src/main.cpp's kMaxCanvasWidth/Height. See that file's
// comment, and the feedback_rt_thread_no_locks / feedback_no_custom_
// concurrency_primitives memories, for why: boost::lockfree::spsc_value
// (the audited SPSC "latest value" triple buffer used for every RT frame/
// config handoff below) default-constructs its 3 internal slots with no way
// to pass a runtime size in, so only compile-time-sized payloads fit
// without the RT thread ever allocating. Configs that exceed this are
// rejected at startup (validate_frame_size), never silently truncated.
  ...
  if (bytes > kMaxFrameBytes) {
    std::cerr << what << ": " << width << "x" << height
               ...
               << kMaxCanvasHeight << "). RT frame buffers are fixed-size for "
                 ...
                 "kMaxCanvasWidth/kMaxCanvasHeight in downstream_main.cpp and "
                 "rebuild.\n";
    return false;
  }
  return true;
...
// Mirrors pw-video-compositor/src/main.cpp's FrameSlot.
struct FrameSlot {
...
// Mirrors pw-video-compositor/src/main.cpp's RenderSlotConfig.
...
struct RenderSlot {
  ...
  RenderSlotConfig control_config; // control-thread-only working copy, published via config_buffer.write()
};
...
void on_input_process(void *data) {
  ...
  if (copy_size > 0) {
    ...
    source.buffer.write(source.write_scratch);
  }
...
void on_output_process(void *data) {
  ...
  // allow_multiple_reads<true>: consume() is wait-free and always succeeds
  // (never a lock, never blocks on on_input_process's scheduling).
  bool base_has_frame = false;
  app.base_video_source.buffer.consume([&](const FrameSlot &slot) {
    base_has_frame = slot.has_frame;
    if (base_has_frame)
...
// Mirrors pw-video-compositor/src/main.cpp's apply_object_params. Runs on
// the control thread - mutates slot.control_config directly (uncontended)
// then publishes a copy via slot.config_buffer.write(). See RenderSlot/
// RenderSlotConfig comments in main.cpp for why this is a spsc_value, not
// a mutex.
...
int main(int argc, char **argv) {
  ...
  // static, not a stack local: App::base_video_source is embedded directly
  // (not via deque), and its spsc_value + write_scratch are now ~32MB fixed
  // (see kMaxFrameBytes above) - well beyond a safe stack frame.
  static App app;
  ...
  if (!validate_frame_size(app.canvas_width, app.canvas_height, "downstream_scene: canvas"))
    return 1;
  ...
  for (size_t idx = 0; idx < input_count; ++idx) {
    ...
    if (!validate_frame_size(src.width, src.height, "downstream_scene: input"))
      return 1;
  }
  ...
  for (size_t scene_i = 0; scene_i < scene_configs.size(); ++scene_i) {
    ...
    scene.render_slots.emplace_back();
    {
      ...
      auto &base_config = base_slot.control_config;
      base_slot.source = &app.base_video_source;
      base_config.dst_x = 0;
      base_config.dst_y = 0;
      base_config.dst_width = app.canvas_width;
      base_config.dst_height = app.canvas_height;
      base_config.scale_x = 1.0;
      base_config.scale_y = 1.0;
      build_sample_maps(base_config, app.canvas_width, app.canvas_height);
      base_slot.config_buffer.write(base_config);
    }
    ...
    for (const auto &object : cfg.objects) {
      ...
      if (object.type == downstream_scene::ObjectType::Video) {
        ...
        if (!validate_frame_size(static_cast<uint32_t>(width), static_cast<uint32_t>(height),
                                 "downstream_scene: image")) {
        ...
        const size_t byte_count = static_cast<size_t>(width) * height * kBytesPerPixel;
        ...
        // Images never get a live producer thread - this one write() at
        // build time (single-threaded, before pw_main_loop_run) is the
        // only publish they'll ever get.
      ...
      // Initial publish: composite_input must see the real layout from its
      // very first call, not config_buffer's default-constructed one.
      slot.config_buffer.write(config);
    }
