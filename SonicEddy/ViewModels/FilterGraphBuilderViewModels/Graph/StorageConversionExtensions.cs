...
{
    public static FilterGraphLv2Plugin ToGrpc(this Lv2PluginNodeViewModel viewModel, Guid id)
    {
        var inputPorts = viewModel.InPorts.OfType<Lv2PortViewModel>()
            .Select(p => new FilterGraphLv2InputPort(p.Id, p.Name, p.Description.Symbol))
            .ToList();
    ...
    {
        var inputPorts = viewModel.InPorts.OfType<PortViewModelBase>()
            .Select(p => new FilterGraphBuiltinPort(p.Id, p.Name))
            .ToList();
        var outputPorts = viewModel.OutPorts.OfType<PortViewModelBase>()
            .Select(p => new FilterGraphBuiltinPort(p.Id, p.Name))
            .ToList();
        var controls = viewModel.Controls
            .Select(c => new FilterGraphBuiltinControl(c.Name, c.Value))
            .ToList();
        return new(id, viewModel.Name, viewModel.NodeType, viewModel.ChannelCount,
            inputPorts, outputPorts, controls);
    }

    public static FilterGraphOutput ToGrpc(this OutputNodeViewModel viewModel, Guid id)
    {
        var ports = viewModel.InPorts.OfType<PortViewModelBase>()
            .Select(p => new FilterGraphOutputInputPort(p.Id, p.Name))
            .ToList();
        return new(id, ports);
    }

    public static FilterGraphInput ToGrpc(this InputNodeViewModel viewModel, Guid id)
    {
        var ports = viewModel.OutPorts.OfType<PortViewModelBase>()
            .Select(p => new FilterGraphInputOutputPort(p.Id, p.Name))
            .ToList();
        return new(id, ports);
    }
    ...
    public static FilterGraph ToGrpc(this FilterGraphBuilderViewModel viewModel)
    {
        ...
        return new(viewModel.Id, viewModel.Name, nodes, edges, flatParameters);
    }
    ...
    private static Guid IdInputPortByIndex(FilterGraphNodeBase node, int index) =>
        node switch
        {
            ...
            FilterGraphBuiltinNode n => n.InputPorts[index].Id,
            _ => throw new NotImplementedException()
        };

    private static Guid IdOutputPortByIndex(FilterGraphNodeBase node, int index) =>
        node switch
        {
            FilterGraphInput n       => n.OutputPorts[index].Id,
            FilterGraphLv2Plugin n   => n.OutputPorts[index].Id,
            FilterGraphBuiltinNode n => n.OutputPorts[index].Id,
            _ => throw new NotImplementedException()
        };

    private static FilterGraphNodeBase BaseToGrpc(this GraphNode viewModel) =>
        viewModel switch
        {
            InputNodeViewModel n     => n.ToGrpc(n.Id),
            OutputNodeViewModel n    => n.ToGrpc(n.Id),
            Lv2PluginNodeViewModel n => n.ToGrpc(n.Id),
            BuiltinNodeViewModel n   => n.ToGrpc(n.Id),
            _ => throw new NotImplementedException()
        };
}
