...
std::tuple<bool, WireplumberThread *, WpIterator *>
WireplumberThread::finish_callback_and_prepare_iterator(GObject *source_object,
                                                        GAsyncResult *res,
                                                        gpointer user_data) {
  const auto this_ = static_cast<WireplumberThread *>(user_data);
...
void WireplumberThread::wp_process_object_enum_props_callback(
  ...
  if (success == false) {
    ...
    this_ = static_cast<WireplumberThread *>(user_data);
...
void WireplumberThread::wp_process_object_enum_prop_info_callback(
    GObject *source_object, GAsyncResult *result, gpointer user_data) {
  auto [success, this_, iterator_pointer] =
      finish_callback_and_prepare_iterator(source_object, result, user_data);
...
void WireplumberThread::g_signal_object_added_callback(
...
void WireplumberThread::g_signal_object_removed_callback(
    WpObjectManager *object_manager, gpointer object, gpointer user_data) {
  const auto this_ = static_cast<WireplumberThread *>(user_data);
...
void WireplumberThread::shutdown_loop() { g_main_loop_quit(_loop); }

void WireplumberThread::setup_object_manager() {
...
int WireplumberThread::setup_wireplumber() {
  wp_init(WP_INIT_ALL);
  ...
  if (!wp_core_connect(_wireplumber_core)) {
    return ERR_COULD_NOT_CONNECT_WIREPLUMBER_CORE;
  }

  return 0;
}

int WireplumberThread::init() {
...
void WireplumberThread::stop() const {
  pthread_kill(_thread->native_handle(), SIGINT);
  _thread->join();
}

void WireplumberThread::run() {
  g_main_loop_run(_loop);
  cleanup();
}

void WireplumberThread::cleanup() {
...
void WireplumberThread::trigger_object_added_callback(
...
void WireplumberThread::trigger_props_changed_callback(
...
void WireplumberThread::trigger_props_enumeration_failed_callback(
...
void WireplumberThread::trigger_object_deleted_callback(
    uint64_t object_serial,
...
void WireplumberThread::g_signal_params_changed_callback(WpPipewireObject *self,
                                                         const gchar *key,
                                                         gpointer user_data) {
  const auto this_ = static_cast<WireplumberThread *>(user_data);
  uint64_t object_serial;
...
auto signal_handler = +[](gpointer user_data) -> gboolean {
  const auto _this = static_cast<WireplumberThread *>(user_data);
  _this->shutdown_loop();
  return G_SOURCE_REMOVE;
};

void WireplumberThread::register_signals() {
...
WpPipewireObject *
WireplumberThread::get_object_by_object_id(uint64_t object_id) const {
...
pw_core *WireplumberThread::pipewire_core() const { return _pipewire_core; }

pw_context *WireplumberThread::pipewire_context() const {
  return _pipewire_context;
}

pw_loop *WireplumberThread::pipewire_loop() const { return _pipewire_loop; }

void WireplumberThread::setup_pipewire() {
...
void WireplumberThread::set_metadata_entry(const char *metadata_name,
...
void WireplumberThread::delete_metadata_entry(const char *metadata_name,
                                              uint64_t subject,
                                              const char *key) {
  const auto metadata =
      _metadata_collection.get_metadata_by_name(metadata_name);
...
WpLink *WireplumberThread::create_link_by_port_id(uint64_t output_port_id,
...
void WireplumberThread::delete_link_object_id(const uint64_t object_id) const {
...
void WireplumberThread::delete_link(WpLink *link) const {
