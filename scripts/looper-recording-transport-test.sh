...
looper_name="${LOOPER_NAME:-se.test_looper_recording_transport}"
looper_tag="${LOOPER_TAG:-test-looper-recording-transport}"
...
"${record_bin}" \
  -n se.test_record_recording_transport \
...
"${signal_bin}" \
  -n se.test_signal_recording_transport \
...
echo "attempting cut/play while transport is stopped"
...
cut_beat="$(scheduled_beat_at "${sync_id}" "${cut_schedule_index}")"
if [[ -z "${cut_beat}" || "${cut_beat}" == "null" ]]; then
  echo "failed to choose synced cut/play beat" >&2
  exit 1
fi

echo "cutting and playing loop 0 at synced beat ${cut_beat}"
pw-cli set-param "${capture_id}" Props \
  "{ params = [ \"commands\" \"[[${cut_beat},\\\"cut 1 0\\\"],[${cut_beat},\\\"play 0\\\"]]\" ] }"
...
if ! awk -v expected="${signal_value}" \
  ...
  END {
    ...
    if (!matched) {
      printf("validation failed: no later record window reached expected playback rms %.9f within %.9f\n",
             expected, tolerance) > "/dev/stderr"
      exit 1
    }
    printf("validation passed: stopped transport stayed silent first, then synced recording cut/play reached rms %.6f within %.6f\n",
           expected, tolerance)
  }
' "${record_windows}"; then
  dump_logs
