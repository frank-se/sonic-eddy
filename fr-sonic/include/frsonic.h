...
namespace models::props  { struct Props; }
namespace models::params { struct ParamUpdate; enum class ParamType; }
...
using node_added_callback_t =
    void (*)(const wireplumber_object *node);

using props_changed_callback_t =
    void (*)(const Props *props, const ParamUpdate *param_update);

using props_enum_failed_callback_t =
    void (*)(uint64_t object_serial);

using prop_info_added_callback_t =
    void (*)(const char *property_infos_json);

using object_deleted_callback_t =
    void (*)(uint64_t object_serial, wireplumber_object_type type);

using metadata_added_callback_t =
    void (*)(const char *metadata_name);

using metadata_entry_updated_callback_t =
    void (*)(const char *metadata_name, uint64_t subject,
...
using channel_select_callback_t   = void (*)(ChannelType channel_type, size_t channel_id);
using dial_mode_callback_t        = void (*)(ChannelType channel_type, size_t channel_id, DialMode dial_mode);
using filter_section_callback_t   = void (*)(ChannelType channel_type, size_t channel_id, size_t section_id);
using pages_right_callback_t      = void (*)(uint64_t step_count);
using pages_left_callback_t       = void (*)(uint64_t step_count);
using midi_cc_update_callback_t   = void (*)(ChannelType channel_type, uint64_t channel_id,
...
FRSONIC_API void frsonic_set_volumes(uint64_t object_id,
                                     const double *volumes, size_t count);
...
FRSONIC_API size_t frsonic_create_fader_fox_pc4_port(
    const char *pmx_purpose, const char *pmx_tag);
